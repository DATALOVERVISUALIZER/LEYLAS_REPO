...
DELETE FROM trfm.tmek_haric_musteri_tmp01 WHERE 1=1;
   INSERT  /*+ APPEND */ INTO trfm.tmek_haric_musteri_tmp01
   select g.tarih, g.ilk7 mus_no,
         MIN (to_date(h.actarih,'dd.mm.yyyy')) min_tmsf_acls_trh
    from extr.fs_gunluk_hplandet_sg g
         inner join extr.fs_hplandet_sg h
                    on g.sube = h.sube
                    and g.sinif = h.sinif
                    ...
                    and g.ekno = h.ekno
                                   where 1 = 1
                                     ...
                                     and g.sube >= 900
                                     and g.durum < 9
group by g.tarih, g.ilk7;

   COMMIT;

      DELETE FROM trfm.tmek_haric_musteri_tmp02 WHERE 1=1;
   INSERT  /*+ APPEND */ INTO trfm.tmek_haric_musteri_tmp02
   select distinct g.tarih, g.ilk7 mus_no
    from extr.fs_gunluk_hplandet_sg g
                                   where 1 = 1
                                     and g.tarih = v_tarih
                                     and g.sinif = 'C'
                                     and SUBSTR (LPAD (g.ana, 3, '0'), 1, 2) = '91'
                                     and g.sube < 900
                                     and case when (g.karakter = 'B' and g.borc > g.alac) or  (g.karakter = 'A' and g.alac > g.borc) then 1 else 0 end = 1
                                     and g.durum < 9;

   COMMIT;

         DELETE FROM trfm.tmek_haric_musteri_tmp03 WHERE 1=1;
   INSERT  /*+ APPEND */ INTO trfm.tmek_haric_musteri_tmp03
   select i.*
    from TRFM.tmek_haric_musteri_tmp01 i
   where 1 = 1
     and not exists (select 1 from TRFM.tmek_haric_musteri_tmp02 j
                          where i.tarih = j.tarih
                          and i.mus_no = j.mus_no);

   COMMIT;

   DELETE FROM trfm.temmekmas_n WHERE 1=1;
   INSERT  /*+ APPEND */ INTO trfm.temmekmas_n
   select t.*, min_tmsf_acls_trh from extr.fs_temmekmas t
              left outer join TRFM.tmek_haric_musteri_tmp03 x
               on t.musteri = x.mus_no;
...
DELETE FROM trfm.kkb_varlik_satis WHERE 1=1 AND kst_trh = v_tarih;
COMMIT;

INSERT   /*+ APPEND */ INTO trfm.kkb_varlik_satis
SELECT DISTINCT k.tarih kst_trh, k.musteri_no
        /*, k.tahsilat_satis_tutari tutar, k.createsicil, k.createtarih, k.createzaman,
       k.createaciklama, k.onayredsicil, k.onayredtarih, k.onayredzaman, k.onayredaciklama,
       k.durum, k.fistarih, k.fissube, k.fissicil, k.fisno, k.updsicil, k.updtarih, k.updzaman*/
FROM extr.fs_varlik_satis_islemleri k
WHERE k.durum IN (1,2)
AND k.fis_sonuc = 'FIS ISLEMI BASARILI'
AND k.tarih <= v_tarih; --Prodda initial olcak
...
COMMIT;
  ...
  SELECT                /*+ parallel (16) */
         ...
         CASE WHEN vys.musteri_no IS NOT NULL THEN 6
              WHEN a.tip IN ('TKRD', 'NKRD', 'KMH') THEN COALESCE (tkrd_hodk.tkrd_rkhsod, nkrd_hodk.rkhsod, kmh_hodk.kmh_rkhsod)
    ...
    FROM trfm.tmp_trbac_fct_01 a
         ...
         LEFT JOIN trfm.tmp_trbac_memzuc_151_152 mmzc
                        ON 1 = 1
                            ...
                            AND a.tip = 'MMZC'
         LEFT JOIN trfm.kkb_varlik_satis vys
                        ON 1=1
                        AND a.kst_trh = vys.kst_trh
                        AND a.musteri_no = vys.musteri_no
   WHERE 1 = 1;
   ...
   BEGIN
      ...
      SELECT a.kst_trh, a.tip, a.musteri_no, a.tck_no, a.vergi_no, a.musteri_tip, a.gercek_tuzel,
             ...
             CASE WHEN a.grup3_alinma_tutari IS NULL THEN '000000000000000'
                  WHEN  a.grup3_alinma_tutari <> 0 THEN LPAD (TO_CHAR (ROUND (TO_NUMBER (a.grup3_alinma_tutari), 0)), 15, '0')
             ...
             CASE WHEN a.grup4_alinma_tutari IS NULL THEN '000000000000000'
                  WHEN a.grup4_alinma_tutari <> 0 THEN LPAD (TO_CHAR (ROUND (TO_NUMBER (a.grup4_alinma_tutari), 0)), 15, '0')
                       ELSE '000000000000000'
             ...
             CASE WHEN a.grup5_alinma_tutari IS NULL THEN '000000000000000'
                  WHEN a.grup5_alinma_tutari <> 0 THEN LPAD (TO_CHAR (ROUND (TO_NUMBER (a.grup5_alinma_tutari), 0)), 15, '0')
                       ELSE '000000000000000'
